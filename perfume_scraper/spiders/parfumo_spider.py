...
class ParfumoSpider(scrapy.Spider):
    ...
    def parse_brand_page(self, response: scrapy.http.Response) -> Generator[Any, None, None]:
        ...
        # Debug log to verify country extraction
        self.logger.debug(f"Extracted country for {brand_name}: {country}")

        # Extract established year
        established = response.css('span.label_a::text').re_first(r'since (\d{4})')
        self.logger.debug(f"Extracted established year for {brand_name}: {established}")
